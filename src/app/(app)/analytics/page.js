...
export default async function AnalyticsPage() {
  ...
  if (!page) {
    ...
    // For example, redirecting back to the homepage or to a account:
    return redirect("/accountrd");
        ...
        {page.links.map((link) => (
          <div key={link.title} className="md:flex gap-4 items-center border-t border-gray-200 py-4">
            ...
            <div className="grow">
              ...
              <a className="text-xs text-blue-400 hover:underline" target="_blank" href={link.url}>
                {link.url}
              </a>
            </div>
            <div className="text-center">
              <div className="border rounded-md p-2 mt-1 md:mt-0">
                <div className="text-3xl">{clicks.filter((c) => c.uri === link.url && isToday(c.createdAt)).length}</div>
